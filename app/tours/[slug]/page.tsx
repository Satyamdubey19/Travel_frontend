...
import { findTourBySlug, type Tour } from "@/lib/tours";
...
function difficultyFor(tour: Tour) {
  if (tour.tags.some((tag) => ["trekking", "mountain", "safari"].includes(tag)))
    return "Moderate";
...
export default function TourDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadTour = async () => {
      try {
        const { data: payload } = await api.get(`/tour/${slug}`, {
        ...
        if (!ignore) setTour(fallback ?? null);
      } catch {
        if (!ignore) setTour(null);
        const fallback = findTourBySlug(slug);
        if (!ignore) setTour(fallback ?? null);
      } finally {
  ...
  useEffect(() => {
    ...
    const loadChatAccess = async () => {
      try {
        await api.get(`/tour/${slug}/chat?scope=participant`, {
          headers: { "Cache-Control": "no-store" },
        });
  ...
  useEffect(() => {
    ...
    const loadBatches = async () => {
      try {
        const { data: payload } = await api.get(`/tour/${slug}/batches`, {
          headers: { "Cache-Control": "no-store" },
        });
        if (!ignore) {
  ...
  useEffect(() => {
    ...
    const loadReviews = async () => {
      try {
        const { data: payload } = await api.get(`/tour/${slug}/reviews`, {
          headers: { "Cache-Control": "no-store" },
        });
        if (!ignore)
          setBackendReviews(Array.isArray(payload?.data) ? payload.data : []);
      } catch {
  ...
  const seatsLeft = selectedBatch
    ? Math.max(0, selectedBatch.seatsLeft)
    : computed.group.left;
  ...
  const visibleReviews = backendReviews.map((review) => ({
  ...
  const joinWaitlist = async () => {
    ...
    setWaitlistMessage("");
    try {
      await api.post(`/tour/${slug}/waitlist`, {
        batchId: selectedBatch?.id,
        seatsRequested: guestCount,
      });
      setWaitlistMessage(
        "You are on the waitlist. We will notify you when seats open.",
      );
    } catch {
      setWaitlistMessage(
        "Could not join waitlist right now. Please try again.",
      );
    } finally {
      ...
      <main className="min-h-screen bg-[#eff1f3] pb-24">
          ...
          <div className="relative mx-auto flex min-h-[58vh] max-w-7xl flex-col justify-end px-4 pb-10 pt-28 sm:min-h-[66vh] sm:px-6 lg:px-8">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <Button
                asChild
                variant="secondary"
                className="rounded-full bg-black/45 text-white ring-1 ring-white/20 hover:bg-black/55"
              >
                <Link href="/tours">
          ...
          <Card className="mx-auto max-w-7xl border border-slate-200 bg-white py-3 shadow-[0_18px_40px_rgba(15,23,42,0.08)]">
            <CardContent className="grid gap-3 px-3 sm:grid-cols-4">
              ...
              <div className="rounded-xl bg-slate-50 p-3">
                ...
                <p className="mt-1 text-sm font-black text-slate-950">
                  {(tour.riskLevel ?? "LOW").replaceAll("_", " ")}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="mx-auto flex max-w-7xl gap-2 overflow-x-auto px-4 py-3 sm:px-6 lg:px-8">
            {navItems.map(([id, label]) => (
              <a
                key={id}
                href={`#${id}`}
                className="shrink-0 rounded-full px-4 py-2 text-sm font-black text-slate-600 transition hover:bg-slate-100 hover:text-slate-950"
              >
                {label}
              </a>
            ))}
        ...
        <section className="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_360px] lg:px-8">
          <div className="space-y-6">
            <Card
              id="overview"
              className="scroll-mt-24 border border-slate-200 bg-white py-4 shadow-sm"
            >
              <CardContent>
                ...
                <h2 className="mt-2 text-3xl font-black tracking-tight text-slate-950">
                  A trip designed for stories and community
                </h2>
                <p className="mt-4 max-w-3xl text-base leading-8 text-slate-700">
                  {tour.description}
                </p>
                <p className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm font-bold text-slate-700">
                  Languages: {tour.languages?.length ? tour.languages.join(", ") : "Ask the organizer before booking"}
                </p>
              </CardContent>
            </Card>

            <Card id="safety" className="scroll-mt-24 overflow-hidden border border-slate-200 bg-white py-0 shadow-sm">
              <div className={`p-5 text-white ${(tour.riskLevel === "HIGH" || tour.riskLevel === "VERY_HIGH") ? "bg-gradient-to-r from-rose-950 to-orange-800" : tour.riskLevel === "MEDIUM" ? "bg-gradient-to-r from-amber-800 to-orange-600" : "bg-gradient-to-r from-slate-950 to-cyan-800"}`}>
                <p className="text-xs font-black uppercase tracking-[0.22em] text-white/70">Risk is not the same as difficulty</p>
                <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
                  <h2 className="text-2xl font-black">{(tour.riskLevel ?? "LOW").replaceAll("_", " ")} risk</h2>
                  <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-black ring-1 ring-white/20">Minimum age {tour.minimumAge ?? 18}</span>
                </div>
              </div>
              <CardContent className="space-y-5 p-5">
                <div>
                  <h3 className="text-sm font-black text-slate-950">What you should understand</h3>
                  <p className="mt-2 text-sm leading-7 text-slate-700">{tour.riskDisclosure}</p>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="rounded-2xl bg-slate-50 p-4">
                    <h3 className="text-xs font-black uppercase tracking-wider text-slate-500">Meeting guidance</h3>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-800">{tour.meetingPoint}</p>
                  </div>
                  <div className="rounded-2xl bg-slate-50 p-4">
                    <h3 className="text-xs font-black uppercase tracking-wider text-slate-500">Access controls</h3>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-800">
                      {tour.verifiedTravelersOnly ? "Supported traveler verification required" : "Standard account checks"}
                      {tour.joinApprovalRequired ? " · Organizer approval required" : ""}
                    </p>
                  </div>
                </div>
                <div className="grid gap-5 sm:grid-cols-2">
                  <div>
                    <h3 className="text-sm font-black text-slate-950">Eligibility</h3>
                    <ul className="mt-2 space-y-2 text-sm text-slate-700">
                      {(tour.eligibilityRequirements ?? []).map((item) => <li key={item} className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-emerald-600" />{item}</li>)}
                    </ul>
                  </div>
                  <div>
                    <h3 className="text-sm font-black text-slate-950">Required equipment</h3>
                    {(tour.requiredEquipment ?? []).length ? (
                      <ul className="mt-2 space-y-2 text-sm text-slate-700">{(tour.requiredEquipment ?? []).map((item) => <li key={item} className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-cyan-700" />{item}</li>)}</ul>
                    ) : <p className="mt-2 text-sm text-slate-500">No specialist equipment listed.</p>}
                  </div>
                </div>
                {tour.emergencyPlanRecorded && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><h3 className="text-sm font-black text-amber-950">Organizer escalation plan recorded</h3><p className="mt-2 text-sm leading-7 text-amber-900">The organizer has recorded stop criteria, communication fallbacks and response steps for review. Sensitive contacts and routes are not published.</p><p className="mt-2 text-xs font-bold text-amber-800">This is operational planning, not medical care or a rescue guarantee.</p></div>}
              </CardContent>
            </Card>
            ...
            >
              <CardContent>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  See the journey before you join
                </p>
                <div className="mt-4 grid gap-3 sm:grid-cols-4">
                  {gallery.slice(0, 5).map((image, index) => (
                    <button
                      ...
                      className={`group relative overflow-hidden rounded-2xl bg-slate-200 ${index === 0 ? "sm:col-span-2 sm:row-span-2" : ""}`}
                    >
                      <img
                        src={image}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
                    ...
                    <ul className="mt-3 space-y-2 text-sm text-emerald-900">
                      {tour.budget.inclusions.map((item) => (
                  ...
                  <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5">
                    ...
                    <ul className="mt-3 space-y-2 text-sm text-rose-900">
                      {tour.budget.exclusions.map((item) => (
                        <li key={item} className="flex items-center gap-2">
                          <X className="h-4 w-4" />
                          {item}
                        </li>
                ...
                <div className="mt-3 flex items-center justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-black text-slate-950">
                      Approved marketplace organizer
                    </h3>
                    <p className="mt-1 text-sm text-slate-600">
                      Host access and this listing were reviewed. Specific credential badges are shown only when their supporting records exist.
                    </p>
                  </div>
                  <Badge className="bg-cyan-100 text-cyan-900">Listing approval recorded</Badge>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="sticky top-24 overflow-hidden border border-slate-200 bg-white py-0 shadow-[0_22px_55px_rgba(15,23,42,0.12)]">
              <CardContent className="space-y-3">
                <div className="-mx-6 -mt-6 border-b border-slate-100 bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 px-6 pb-5 pt-6 text-white">
                  ...
                  <p className="mt-5 text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100">
                    Starting from
                  </p>
                  <div className="mt-1 flex items-end justify-between">
                    ...
                    <p className="pb-1 text-xs font-semibold text-cyan-100">
                      per person
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-black text-slate-950">
                      {tour.duration} days - {computed.difficulty}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {batches.slice(0, 3).map((batch) => {
                        ...
                        const batchSoldOut =
                          batch.seatsLeft <= 0 || batch.status === "SOLD_OUT";
                        return (
                          <button
                            key={batch.id}
                            onClick={() => {
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <p className="flex items-center gap-1.5 text-sm font-black text-slate-950">
                                  ...
                                  {formatShortDate(new Date(batch.startDate))} -{" "}
                                  {formatShortDate(new Date(batch.endDate))}
                                </p>
                                <p className="mt-1 text-xs font-semibold text-slate-500">
                                  {batch.label || "Curated group departure"}
                                </p>
                              </div>
                              <Badge
                                className={
                            ...
                            <p className="mt-2 text-sm font-black text-slate-950">
                              {formatMoney(batch.currentPrice)}
                            </p>
                          </button>
                        );
                      })}
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setGuestCount(
                            Math.min(Math.max(1, seatsLeft), guestCount + 1),
                          )
                        }
                        disabled={isSoldOut}
                      >
                  ...
                  <p className="mt-2 text-xs font-semibold text-slate-500">
                    Add full traveler, ID, medical and emergency details in the
                    next step.
                  </p>
                </div>

                <Button
                  onClick={isSoldOut ? joinWaitlist : openBookingWizard}
                  disabled={joiningWaitlist}
                  className={`h-12 w-full rounded-2xl text-base font-black shadow-lg ${isSoldOut ? "bg-amber-600 text-white shadow-amber-200 hover:bg-amber-700" : "bg-slate-950 text-white shadow-slate-200 hover:bg-cyan-700"}`}
                >
                  {isSoldOut
                    ? "Join waitlist"
                    : computed.approvalRequired
                      ? "Request to join"
                      : "Book this trip"}
                </Button>

                <p className="text-center text-xs font-semibold text-slate-500">
                  Seats are revalidated before payment
                </p>
                {waitlistMessage ? (
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button variant="outline" onClick={() => setLiked(!liked)}>
                    <Heart
                      className={`mr-1 h-4 w-4 ${liked ? "fill-rose-500 text-rose-500" : ""}`}
                    />
                    Save
                  </Button>
                ...
                <div className="rounded-2xl border border-cyan-200 bg-cyan-50 p-3 text-xs font-bold text-cyan-900">
                  Only {seatsLeft} slots left for this departure. Group bookings
                  stay together if seats are available.
                </div>
                {computed.approvalRequired ? (
                  <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-800">
                    <Lock className="mr-1 inline h-3.5 w-3.5" />
                    Host approval required for this tour.
                  </div>
                ) : null}
                {chatUnlocked ? (
                  <Button
                  ...
                  >
                    <Link href={`/tours/${tour.slug}/chat`}>
                      Open Group Chat
                    </Link>
                  </Button>
                ) : null}
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white p-3 shadow-[0_-18px_45px_rgba(15,23,42,0.1)] lg:hidden">
          <div className="mx-auto flex max-w-7xl items-center gap-3">
            ...
            >
              <Heart
                className={`h-5 w-5 ${liked ? "fill-rose-500 text-rose-500" : "text-slate-600"}`}
              />
            </Button>
            <Button
              onClick={isSoldOut ? joinWaitlist : openBookingWizard}
              className={`${isSoldOut ? "bg-amber-600 text-white hover:bg-amber-700" : "bg-teal-700 text-white hover:bg-teal-800"}`}
            >
              {isSoldOut
                ? "Waitlist"
                : computed.approvalRequired
        ...
        {lightbox ? (
          ...
          >
            <Button
              size="icon"
              variant="secondary"
              className="absolute right-5 top-5 bg-white text-slate-950 hover:bg-slate-200"
            >
              <X className="h-5 w-5" />
            </Button>
            <img
        ...
        <TourBookingWizard
          ...
          safety={{
            riskLevel: tour.riskLevel ?? "LOW",
            riskDisclosure: tour.riskDisclosure ?? "Risk information is unavailable. Do not continue; contact support.",
            minimumAge: tour.minimumAge ?? 18,
          }}
          contact={{
            name: user?.name ?? "",
            email: user?.email ?? "",
